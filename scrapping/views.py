...
import requests, ast, urllib, time, simplejson,cookielib
...
link = {
	...
	"gustazos":{
		"link"   : "http://www.gustazos.com/?utm_source=splashpage&utm_medium=website&utm_campaign=2012-10-SplashPageCity-PR",
...
def getGroupon(start,end,index, company_list):
	...
	# 	website = link.get(company)
	# 	d = pq("http://www.groupon.com.pr/descuentos/all")

	# 	if start is 0:
	# 		first = d.find(".first-deal")
	# 		print d("div").find(".first-deal").html()
	# 		print d.html()
	# 		items.append({
	# 			'from' : "oferta",
	# 			"title": d(".name").text(),
	...
	# 		})
		...
		if start is 0:
			...
			items.append({
				...
				"link" : "http://www.gustazos.com" + d(".summary").find("a").attr["href"] ,
		...
		for item in elements[start:end]:
			...
			items.append({
				...
				"link" : "http://www.gustazos.com%s" % (itemEl.find(".company").attr["href"]),
		...
		if start is 0:
			...
			items.append({
				...
				"text" : first(".description").children("p").text(),
				"image": first(".slideshow").find("img").attr["src"] ,
				"link" : "http://www.groupon.com.pr%s" % (first(".title").attr["href"]) ,
